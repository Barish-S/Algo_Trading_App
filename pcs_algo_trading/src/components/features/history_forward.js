...
const Historical_Forward = () => {
  ...
  const fetchData = () => {
    axios.get('http://127.0.0.1:5000/history-forward')
      .then((res) => {
  ...
  const generatePDF = () => {
    ...
    html2canvas(input, { scale: 2, useCORS: true, logging: true })
      .then((canvas) => {
        ...
        pdf.save(`Historical_Forward_${getCurrentDateTime()}.pdf`);
      })
      .catch((error) => {
  ...
  const generateExcel = () => {
    ...
    // Save to file
    XLSX.writeFile(wb, `Historical_Forward_${getCurrentDateTime()}.xlsx`);
  };
  ...
  const clearTable = () => {
    ...
    formData.append("request", "history-forward");
  ...
  return (
    <>
        <div style={{display:"flex",justifyContent:"center",alignItems:"center",gap:"10px"}}>
          ...
          {globeStatus=="history_forward"?<Button type='button' variant='outline-dark' onClick={()=>dispatch(setPageStatus("history_reverse"))}>Reverse Datas</Button>:<Button type='button' variant='danger' onClick={()=>dispatch(setPageStatus("history_forward"))}>Forward Datas</Button>}
        </div>
        <br />
        <hr></hr>
...
export default Historical_Forward;
